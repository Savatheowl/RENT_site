...
from wtforms import StringField, PasswordField, SubmitField, TextAreaField, IntegerField, FloatField, SelectField, FileField, HiddenField
...
class RegisterForm(FlaskForm):
    ...
    role = SelectField('Роль', choices=[('tenant', 'Арендатор'), ('landlord', 'Арендодатель')])
...
class PropertyForm(FlaskForm):
    ...
    price_type = SelectField('Тип оплаты', choices=[
        ('month', 'в месяц'), ('day', 'в сутки'), ('bed', 'койко-место')
    ])
    property_type = SelectField('Тип жилья', choices=[
...
class RequestForm(FlaskForm):
    message = TextAreaField('Сообщение арендодателю', validators=[Optional(), Length(max=500)])
    submit = SubmitField('Отправить заявку')


class ReviewForm(FlaskForm):
    rating = SelectField('Оценка', choices=[(5, '5'), (4, '4'), (3, '3'), (2, '2'), (1, '1')], coerce=int)
